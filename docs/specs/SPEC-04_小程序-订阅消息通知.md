...
| 退款业务闭环（审核+执行退款+微信退款回调） | 🔄 进行中 | 当前仅有退款通知链路，实际退款主流程尚未完成 |
| 管理端退款按钮与审核操作页联动 | ⏳ 未开始 | 待退款业务闭环完成后接入 |

> 备注：本看板用于同步“规格 vs 实现”现状。退款通知能力已可运行，但“审核与退款操作”仍需后续开发完成。
