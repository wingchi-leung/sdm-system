...
| 2026-06-15 v1.2 | 实施完成：后端新增独立表 `community_channel_announcement` + 5 个 API（A1-A5）+ 删频道级联清理 `deleted_announcements`；前端新增 `community-announcement-list/detail/create` 三页 + 改造 `community-post-list` 顶部公告栏入口卡片与「+ 公告」按钮；后端 19 个新测试全过 + 31 个旧测试无回归，前端 8 个新测试全过 + 10 个相邻测试无回归 | commit `94ce64b`（后端）+ 前端批次（紧随其后） |
