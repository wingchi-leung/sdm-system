...
| 社区动态内页改版 | 社区频道动态内页按设计稿重排中间内容区，采用公告标题区、圆角动态卡片、三列图片、展开全文、评论预览和悬浮发布入口；顶部页头与底部 tab 保持原实现，点赞能力暂不接入 | ✅ 已实现 | `miniprogram/pages/community-post-list/community-post-list.wxml`, `miniprogram/pages/community-post-list/community-post-list.wxss` |
...
| 日期 | 变更内容 | 关联文档 |
|------|----------|----------|
| 2026-07-14 | 新增 Teams 社区内容导出工具：通过已登录 Chrome 会话遍历社区主帖及完整回复，仅保留作者名包含 `Inc` 的内容；支持按社区保存检查点、分批增量跳过未变化帖子、按帖子标题建立独立目录，下载文字和图片，并生成原始归档 JSON、Markdown 与适配小程序社区字段的导入草稿；打开对话时会居中帖子、重新定位虚拟列表按钮并在超时后自动重试一次 | `tools/teams-community-crawler/` |
| 2026-07-14 | 新增 Teams 社区归档导入命令：将导出主帖映射为小程序频道帖子、回复映射为评论并复制本地图片；界面只显示原正文和图片，不追加 Teams 作者/时间或纯图片占位文案，并过滤图片宫格产生的 `+1`、`+2` 等角标；支持 dry-run、限制试导入数量、按 Teams 帖子及回复 ID 幂等增量导入和单线程事务回滚 | `backend/scripts/import_teams_community.py`, `backend/tests/scripts/test_import_teams_community.py` |
| 2026-07-14 | 社区频道帖子新增删除接口 `DELETE /api/v1/community/channels/{channel_id}/posts/{post_id}`：帖子作者或频道管理员可删除，其他成员返回 403；帖子及所属评论统一软删除为 `-2`，审核回调不得恢复已删除内容；小程序 API 层同步提供调用方法 | `backend/app/api/v1/endpoints/community.py`, `backend/app/crud/crud_community_channel.py`, `backend/tests/api/test_community_channels.py`, `miniprogram/utils/api.js` |
| 2026-07-14 | 小程序社区富文本解析在渲染前移除 HTML 注释，列表页与详情页不再把解析后的空正文回退成原始字符串；Teams 导入用于幂等去重的来源标记继续保留在数据库但不再显示为正文 | `miniprogram/utils/community-content.js`, `miniprogram/pages/community-post-list/community-post-list.js`, `miniprogram/pages/community-post-detail/community-post-detail.js`, `miniprogram/tests/community-content.test.js`, `miniprogram/tests/community-post-list-page.test.js` |
