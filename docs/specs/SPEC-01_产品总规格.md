...
| 2026-07-14 | 新增 Teams 社区归档导入命令：将导出主帖映射为小程序频道帖子、回复映射为评论，复制本地图片并保留原作者/发布时间；支持 dry-run、限制试导入数量、按 Teams 帖子及回复 ID 幂等增量导入和单线程事务回滚 | `backend/scripts/import_teams_community.py`, `backend/tests/scripts/test_import_teams_community.py` |
