...
| 退款主流程接口（管理员发起退款） | ✅ 已完成 | 已实现 `POST /payments/{order_no}/refund`，支持幂等键与全额退款 |
| 报名审核接口（approve/reject） | ✅ 已完成 | 拒绝后已进入待退款状态 |
| 微信退款申请调用 | ✅ 已完成 | 已接入微信支付退款调用与失败处理 |
| 微信退款回调处理 | ✅ 已完成 | 已实现退款回调幂等落库与通知联动 |
| 管理端退款按钮 | ✅ 已完成 | 小程序报名管理页已支持管理员点击退款 |
...
当前链路是“先支付再审核”。当审核不通过时，管理员可在报名管理页直接发起退款，系统会走微信退款接口、回调落库和退款通知，避免重复退款与财务对账风险。
