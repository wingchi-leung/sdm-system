...
def get_admin_scope(
    ...
    """
    is_super, allowed_ids = crud_admin.get_admin_scope(db, ctx.user_id)
    return {
...
def create_participant(
    ...
    # 检查活动是否存在
    activity = crud_activity.get_activity(db, body.activity_id, tenant_id)
    if not activity:
        raise HTTPException(status_code=404, detail="活动不存在")
...
def list_participants(
    ...
    return crud_participant.get_participants_by_activity(
        db, activity_id, tenant_id, skip=skip, limit=limit
    )
```
