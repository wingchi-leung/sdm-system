...
可将 `tools/teams-community-crawler` 生成的批次目录导入本机社区频道。Teams 主帖会成为频道帖子，筛选出的回复会成为该帖评论，图片复制到本机社区上传目录。界面内容只保留原正文和图片，不额外显示 Teams 作者、时间或“图片回复”占位文字；图片宫格产生的 `+1`、`+2` 等角标也会自动清除。命令默认仅校验，提供 `--apply` 才会写入数据库。
