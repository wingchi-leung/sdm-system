...
- **学习活动全生命周期**：活动发布、报名已支持；签到在管理端 Web 完成。
- **平台人员管理**：注册/不注册报名、黑名单拦截由后端实现，小程序沿用同一套接口。
- **登录/注册**：手机号+密码、双角色；已支持**微信授权登录**（登录页「微信一键登录」，后端需配置小程序 AppID/Secret 并执行 user 表 `wx_openid` 迁移）。
- **美观**：全局样式参考现代 UI（teams/即刻/微信），主色与 Flutter 一致（#007AFF）。
