...
test('活动详情页在权限接口失败时仍应展示活动主体内容', async () => {
  const pageConfig = loadActivityDetailPage({
    api: {
      getActivity() {
        return Promise.resolve({
      ...
      getActivityPermissions() {
        return Promise.reject(new Error('权限接口失败'));
      },
    },
    auth: {
      isLoggedIn: () => true,
  ...
  assert.equal(page.data.activity.activity_name, '未来设计');
  assert.equal(page.data.showAdminPanel, false);
  assert.equal(page.data.showCommunitySection, false);
});

test('活动详情页在报名状态接口失败时仍应展示活动主体内容', async () => {
  const pageConfig = loadActivityDetailPage({
    api: {
      getActivity() {
        return Promise.resolve({
      ...
      getMyParticipantActivities() {
        return Promise.reject(new Error('报名状态接口失败'));
      },
    },
    auth: {
      isLoggedIn: () => true,
  ...
  assert.equal(page.data.activity.activity_name, '未来设计');
  assert.equal(page.data.hasRegistered, false);
  assert.equal(page.data.hasPendingPayment, false);
});
...
test('活动详情页在图片模块缺少 resolveDisplayUrl 时仍可正常展示', async () => {
  const pageConfig = loadActivityDetailPage({
    api: {
      getActivity() {
        return Promise.resolve({
          ...
          activity_name: '未来设计',
          activity_intro: '重新理解人与技术的关系',
          poster_url: '/uploads/posters/poster-a.jpg',
        });
      },
      getActivityPermissions() {
        return Promise.resolve({ can_manage: false });
  ...
  assert.equal(page.data.loading, false);
  assert.equal(page.data.error, null);
  assert.equal(page.data.activity.poster_url, '/uploads/posters/poster-a.jpg');
});
