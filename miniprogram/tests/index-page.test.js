...
test('首页在认证快照失效时会直接踢回登录页', async () => {
  let reLaunchCalls = 0;
  const pageConfig = loadIndexPage({
    api: {
      getEnrollableActivities: () => Promise.resolve({ items: [] }),
      getMyParticipantActivities: () => Promise.resolve({ items: [] }),
      getUserProfile: () => Promise.reject({ statusCode: 401, message: '请先登录' }),
      getAuthSnapshot: () => Promise.reject({ statusCode: 401, message: '请先登录' }),
      getCommunityNotificationUnreadCount: () => Promise.resolve({ unread_count: 0 }),
    },
    auth: {
      isLoggedIn: () => true,
      isAdmin: () => false,
  ...
  assert.equal(reLaunchCalls >= 1, true);
});

test('首页会按开始时间倒序排序活动并保留展示字段', async () => {
  ...
  assert.equal(page.data.visibleActivities[0].activity_name, '明天的活动');
  assert.equal(page.data.visibleActivities[0].location_display, '上海');
  assert.equal(page.data.visibleActivities[0].participant_display, '12 人参加');
  assert.equal(page.data.visibleActivities[1].activity_name, '今天的活动');
});
...
test('首页会按日期分组展示活动标题，便于滚动时定位当前日期', async () => {
  ...
  assert.equal(page.data.dateGroups[0].date_label, '6月10日');
  assert.equal(page.data.dateGroups[0].activities.length, 1);
  assert.equal(page.data.dateGroups[1].date_label, '6月9日');
  assert.equal(page.data.dateGroups[1].activities.length, 2);
});
...
test('首页会把报名处理中状态标记为报名处理中而不是已报名', async () => {
  const pageConfig = loadIndexPage({
    api: {
      getEnrollableActivities: () => Promise.resolve({
        items: [
  ...
  assert.equal(page.data.activities[0].registration_status_text, '报名处理中');
});

test('首页首次进入时不会在 onLoad 和 onShow 重复请求用户资料', async () => {
  const calls = {
    getEnrollableActivities: 0,
  ...
  assert.equal(calls.getEnrollableActivities, 1);
